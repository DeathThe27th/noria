...
export default async function AgentPassport({ params }: { params: Promise<{ chainId: string; tokenId: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-white px-5 pb-36 pt-28 text-[#111] sm:px-8">
      <div className="noria-scale mx-auto max-w-[1040px]">
        <div className="flex items-center justify-between rounded-full bg-[#f3f5f8] p-2 pl-4">
          <Link href="/" className="flex items-center gap-3 font-semibold"><span className="flex size-10 items-center justify-center rounded-full bg-[#111111] text-white"><NoriaMark className="size-5" /></span>Noria</Link>
          <div className="flex items-center gap-2"><Link href={`/missions/new?agent=${agent.token_id}`} className="rounded-full bg-[#111] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#111111]">Create job</Link><a href={sourceUrl} target="_blank" rel="noreferrer" className="hidden rounded-full bg-white px-5 py-3 text-sm font-semibold sm:block">Source ↗</a></div>
        </div>
        ...
        <section className="mt-10">
          <div className="flex items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.14em] text-[#111111]">Service catalog</p><h2 className="mt-2 text-3xl font-semibold tracking-[-.035em]">Services</h2></div><span className="text-xs text-black/45">Pricing not published</span></div>
          <div className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{services.length ? services.map(({ name, endpoint, verified }) => <article key={name} className="y2k-window rounded-[15px] p-5"><div className="flex items-center justify-between"><h3 className="font-semibold">{name}</h3><span className={verified ? "rounded-full bg-black px-2.5 py-1 text-[10px] font-semibold text-white" : "rounded-full bg-[#f2f2f2] px-2.5 py-1 text-[10px] text-black/45"}>{verified ? "Verified endpoint" : "Unverified"}</span></div><p className="mt-4 text-sm text-black/50">{endpoint.host}</p><div className="mt-6 flex items-center justify-between"><span className="text-xs text-black/40">Price unavailable</span><Link href={`/missions/new?agent=${agent.token_id}`} className="rounded-[10px] bg-[#111023] px-4 py-2.5 text-xs font-semibold text-white">Create job</Link></div></article>) : <article className="rounded-[18px] border border-black/10 bg-white p-6 md:col-span-2 lg:col-span-3"><h3 className="font-semibold">No service offer published</h3><p className="mt-2 text-sm text-black/50">This agent has an indexed identity, but no HTTPS service endpoint or price is available.</p></article>}</div>
        </section>
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
          ...
          <section className="y2k-window rounded-[20px] p-7">
            ...
            <div className="mt-6 flex flex-wrap gap-2">{(agent.supported_protocols ?? []).length ? agent.supported_protocols?.map((item) => <span key={item} className="rounded-full bg-[#f1f1f1] px-3 py-1.5 text-xs text-[#111111]">{item}</span>) : <span className="text-sm text-[#8c8089]">No protocols published</span>}</div>
            <div className="mt-7 space-y-3">{services.length ? services.map(({ name, endpoint, verified }) => verified ? <a key={name} href={endpoint.url} target="_blank" rel="noreferrer" className="flex items-center justify-between gap-4 rounded-[18px] border border-black/10 bg-white p-3 text-sm text-[#111] transition hover:border-[#668d7b]"><span>{name}</span><span className="text-right text-xs">Verified · {endpoint.host} ↗</span></a> : <div key={name} className="flex items-center justify-between gap-4 rounded-full border border-black/8 bg-white p-3 text-sm text-black/50"><span>{name}</span><span className="text-right text-xs text-[#8c8089]">Unverified · {endpoint.host}</span></div>) : <p className="text-sm leading-6 text-[#8c8089]">No HTTPS service endpoint is published for this agent.</p>}</div>
          </section>
        </div>
