...
export default async function AgentPassport({ params }: { params: Promise<{ chainId: string; tokenId: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-white px-5 pb-36 pt-6 text-[#111] sm:px-8">
      <div className="noria-scale mx-auto max-w-[1040px]">
        <div className="flex items-center justify-between rounded-full bg-[#f3f5f8] p-2 pl-4">
          <Link href="/" className="flex items-center gap-3 font-semibold"><span className="flex size-10 items-center justify-center rounded-full bg-[#2864ff] text-white"><NoriaMark className="size-5" /></span>Noria</Link>
          <div className="flex items-center gap-2"><Link href={`/missions/new?agent=${agent.token_id}`} className="rounded-full bg-[#111] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#2864ff]">Create job</Link><a href={sourceUrl} target="_blank" rel="noreferrer" className="hidden rounded-full bg-white px-5 py-3 text-sm font-semibold sm:block">Source ↗</a></div>
        </div>
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
          ...
          <section className="rounded-[32px] border border-black/8 bg-[#f6f8fb] p-7">
            ...
            <div className="mt-6 flex flex-wrap gap-2">{(agent.supported_protocols ?? []).length ? agent.supported_protocols?.map((item) => <span key={item} className="rounded-full bg-[#eef2ff] px-3 py-1.5 text-xs text-[#2864ff]">{item}</span>) : <span className="text-sm text-[#8c8089]">No protocols published</span>}</div>
