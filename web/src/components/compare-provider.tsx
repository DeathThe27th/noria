...
export function CompareButton({ agent }: { agent: AgentRecord }) {
  ...
  return <button type="button" onClick={() => context.toggle(agent)} className={selected ? "rounded-full bg-[#f1f1f1] px-4 py-2.5 text-xs font-semibold text-[#111111]" : "rounded-full bg-[#f3f5f8] px-4 py-2.5 text-xs text-black/50 transition hover:bg-[#e7ecf4] hover:text-[#111]"}>{selected ? "Added" : "Compare"}</button>;
}
